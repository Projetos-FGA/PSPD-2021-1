...
// =============================================================================
    ...
    END = omp_get_wtime();
    printf("Time bloco inicial = %f seconds\n", END - START);
    START = omp_get_wtime();
...
//==============================================================================
    ...
    END = omp_get_wtime();
    printf("Time ler_arquivo = %f seconds\n", END - START);
    START = omp_get_wtime();
    
    unificar_resultado();

    END = omp_get_wtime();
    printf("Time unificar_resultado = %f seconds\n", END - START);
    START = omp_get_wtime();

    ordenar_printar(sfe);

    END = omp_get_wtime();
    printf("Time ordenar_printar = %f seconds\n", END - START);
...
void ler_arquivo(int file_size, int l1, char *arquivo)
{
    int i, id_th, voto, bloco, inicio, fim, buf_size, buf_size_th;
    char *token = NULL; 
    char *rest = NULL;
    ...
    bloco = (file_size - l1) / num_th;   
    inicio = (bloco * id_th) + l1;  
